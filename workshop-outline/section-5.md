...
1. Open `includes/summarizer.php` and **replace** the `wcpt_enqueue_editor_assets` function (and its `add_action` call) from Section 4 with this script-module enqueue:
...
add_action( 'enqueue_block_editor_assets', 'wcpt_enqueue_script_modules' );
```
